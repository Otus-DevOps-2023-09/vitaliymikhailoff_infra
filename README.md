...
Установлен плагин **yc_compute**  
Создан файл с настройками парсинга inventory **inventory_yc.yaml**  
Добавлены дополнительные параметры в конфиг **ansible.cfg**  
При выполнении ansible-inventory --list --yaml получаем актуальный инвернтори файл, который парсит данные с YC по тэгу  
Пример получаемого инвентори **inventory_from_plugin.yml**  

Созданы плейбуки **packer_app.yml** и **packer_db.yml**  
Изменен провижининг в конфигах packer для **app.json** и **db.json**  
Созданы новые образы, развернуты терраформом ВМ из новых шаблонов, проверена работоспособность.  

## Ansible роли, управление настройками нескольких окружений и best practices  

### Основное задание:  
Перенесены созданные плейбуки в раздельные роли 
Описаны роли для двух окружений 
Использована коммьюнити роль nginx 
Использован Ansible Vault для создания дополнительных пользователей на серверах

## Локальная разработка Ansible ролей с Vagrant. Тестирование конфигурации. 

### Основное задание:  
Установлен vagrant и VM box
Развернуты ВМ при помощи Vagrant
Выполнена настройка ВМ при помощи плейбуков через провижининг vagrant
Установлен Molecule, протестированы роли на тестовой ВМ
